...
#' @param AUDIO_OUT_DIR Path to output synthetic training data. Defaults to current working directory/symthetic_dataset if not provided and no custom config file is used.
...
ecoVAD.genSynth <- function(configPath, AUDIO_OUT_DIR, ...) {
  if(missing(configPath)){
    if(missing(AUDIO_OUT_DIR)){
      AUDIO_OUT_DIR = file.path(getwd(), "synthetic_data")
    }
    configPath <- file.path(system.file("ecoVAD_chirpR", package = "chirpR"), "config_data.yaml")
